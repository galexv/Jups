...
int RT_Emit_3D(double PHASE)
{
    ...
    *phi_plus_e, *phi_plus_w, *phi_minus_e, *phi_minus_w, **dphi, *theta_lon, *theta_lat, *phi_lon, *dlat_rad, *dlon_rad;
    ...
    int i, j, k, l, m, n, o, c, g, h, ii, kmin;
    ...
    /*Allocate memory*/
    TMI_big_array = malloc(NLAT*sizeof(double));
    for(l=0; l<NLAT; l++)
    {
        TMI_big_array[l] = malloc(NLON*sizeof(double));
        for(m=0; m<NLON; m++)
        {
            TMI_big_array[l][m] = malloc(NTAU*sizeof(double));
        }
    }
        ...
        for(l=0; l<NLAT; l++)
        {
            for(m=0; m<NLON; m++)
            {
                ...
                { 
                    ...
                    kmin=0;
                    while(temperature_3d[l][m][kmin] < 10.0)
                    {
                        kmin += 1;
                    }

                    //printf("%d %d %.8e %.8e\n", l, kmin, temperature_3d[l][m][kmin], temperature_3d[l][m][kmin-1]);

                    //two_stream(NTAU, kmin, PI0_MgSiO3, G0_MgSiO3, atmos.T_3d[l][m], tau_em[l][m], \
                    //CLIGHT / atmos.lambda[i], (CLIGHT / atmos.lambda[i]) - (CLIGHT / atmos.lambda[i+1]), TMI, mean_intensity_top);
                    //TMI_big_array[l][m] = TMI;

                    mean_intensity_top = two_stream(NTAU, kmin, 0.0, 0.0, atmos.T_3d[l][m], tau_em[l][m], 1.0e+13, 100.0, TMI);
                    printf("%.8e in the normal program \n", mean_intensity_top);
