                ...
                if(atmos.lon[m]>=450.0-PHASE && atmos.lon[m]<=630.0-PHASE)
                {
                    for(j=0; j<NTAU; j++)
                    {
                        ...
                        /* No Doppler */
                            ...
                            //if (kappa_nu > 1e20)
                            //{
                            //    printf("%d %d %.8e %.8e %.8e %.8e\n", l, m, opac.T[g], opac.T[g+1], opac.P[h], opac.P[h+1]);
                            //}
                        }
                        ...
                        l = 40;
                        m = 74;
                        printf("%d %.8e %.8e %.8e %.8e\n",j, tau_em[l][m][j], dtau_em[l][m][j], kappa_nu, dl[l][m][j]);
        ...
        for(l=0; l<NLAT; l++)
        {
            for(m=0; m<NLON; m++)
            {
                if(atmos.lon[m]>=450.0-PHASE && atmos.lon[m]<=630.0-PHASE)
                {
                    ...
                    //for(j=0; j<NTAU; j++)
                    //{   
                    //    printf("%.8e %.8e %.8e %.8e\n", atmos.lat[l], atmos.lon[m], atmos.T_3d[l][m][j], tau_em[l][m][j]);
                    //}
                    
                    //printf("CLIGHT / atmos.lambda[i] %.8e  %.8e \n", CLIGHT / 5e-7, atmos.lambda[i]);
                    ...
                    malsky_intensity[l][m] = two_stream(NTAU, kmin, 0.00, 0.00, atmos.T_3d[l][m], tau_em[l][m], \
                    CLIGHT / 5e-7, CLIGHT / atmos.lambda[i] - CLIGHT / atmos.lambda[i+1], TMI, incident_frac);
